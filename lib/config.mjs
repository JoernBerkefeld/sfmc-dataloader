...
export function resolveCredentialAndMid(mcdevrc, mcdevAuth, credentialName, buName) {
    ...
    const mid = typeof midRaw === 'number' ? midRaw : Math.trunc(Number(midRaw));
...
export function buildSdkOptions(logger = null) {
    ...
    const options = {
        ...
        eventHandlers: {
            ...
            onConnectionError: (ex, remainingAttempts) => {
                const endpointString = ex.endpoint ? String(ex.endpoint) : '';
                const endpointSuffix = endpointString
                    ? ` - ${endpointString.split('rest.marketingcloudapis.com', 2)[1] ?? endpointString}`
                    : '';
                log.warn(
    ...
    if (logger) {
        options.eventHandlers = {
            ...options.eventHandlers,
            logRequest: (request) => {
                const message = structuredClone(request);
                if (message.headers?.Authorization) {
                    message.headers.Authorization = 'Bearer *** TOKEN REMOVED ***';
                }
                logger.write(
                    `API REQUEST >> ${message.method?.toUpperCase() || 'GET'} ${message.url}`,
                );
                if (message.data) {
                    const body =
            ...
            logResponse: (response) => {
                logger.write(
                    `API RESPONSE << ${response.status || response.statusCode || '(no status)'}`,
                );
                const body =
