...
function ensureGitignore(projectRoot) {
    ...
    if (lines.every((line) => line.trim() !== entry)) {
 ...
 */
export async function runMcdataInit(options) {
    const { projectRoot, isTTY, yes = false, _buFetcher } = options;
    const out = options.stdout ?? ((message) => console.log(message));
    const error = options.stderr ?? ((message) => log.error(message));
    ...
    if (!yes && (fs.existsSync(mcdataRcPath) || fs.existsSync(mcdataAuthPath))) {
        if (isTTY) {
            const confirmFunction = options._confirm ?? defaultConfirm;
    ...
    // Collect credentials
    let credentialName = options.credential;
    let clientId = options.clientId;
    let clientSecret = options.clientSecret;
    let authUrl = options.authUrl;
    let enterpriseIdString = options.enterpriseId;

    if (
        isTTY &&
        (!credentialName || !clientId || !clientSecret || !authUrl || !enterpriseIdString)
    ) {
        const rl = readline.createInterface({ input, output });
        try {
            if (!credentialName) {
            ...
            if (!enterpriseIdString) {
                enterpriseIdString = (await rl.question('Enterprise MID: ')).trim();
            }
        } finally {
            rl.close();
        }
    ...
    const enterpriseId = Math.trunc(Number(enterpriseIdString));
    if (!Number.isSafeInteger(enterpriseId) || enterpriseId <= 0) {
        error(`Invalid enterprise MID: ${enterpriseIdString}`);
        return 1;
    }
    ...
    try {
        ...
        error(`Failed to fetch Business Units: ${ex.message}`);
        return 1;
    }
