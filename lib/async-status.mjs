...
/**
 * Polls the async import status API for a single requestId until the job reaches
 * a terminal state or a status check fails.
 *
 * @param {{ rest: { get: Function } }} sdk
 * @param {string} requestId - the async request id to poll
 * @returns {Promise.<string|undefined>} the terminal requestStatus (undefined if a status check failed)
 */
async function pollRequestStatus(sdk, requestId) {
    let requestStatus;
    do {
        let statusResult;
        try {
        ...
        if (PENDING_STATUSES.has(requestStatus)) {
            log.info(`Async import still in progress (requestId: ${requestId}) — retrying in 5 s…`);
...
export async function pollAsyncImportCompletion(sdk, requestIds) {
    ...
    for (const requestId of requestIds) {
        ...
        const requestStatus = await pollRequestStatus(sdk, requestId);
