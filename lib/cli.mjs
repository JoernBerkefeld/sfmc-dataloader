...
export async function main(argv) {
    ...
    const logger = values.debug
        ...
        : null;
    if (logger) {
        log.info(`Debug log: "${path.resolve(logger.logPath)}"`);
    }
    ...
    if (sub === 'import') {
        ...
        if (hasDe) {
            ...
            for (const deKey of deKeys) {
                ...
                if (countAfter === null) {
                    log.warn(`Could not verify import result for DE "${deKey}".`);
        ...
        for (const customerKey of keysFromFiles) {
            ...
            if (countAfter === null) {
                log.warn(`Could not verify import result for DE "${customerKey}".`);
            } else {
                const expected =
                    mode === 'insert' || countBefore === 0 ? (countBefore ?? 0) + n : n;
                if (countAfter < expected) {
