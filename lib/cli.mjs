...
function readCliPackageVersion() {
    ...
    return typeof package_.version === 'string' ? package_.version : '';
}
...
export async function main(argv) {
    ...
    const isUseGit = values.git === true;
    ...
    if (sub === 'import') {
        ...
        if (hasDe) {
            const deKeys = [values.de ?? []].flat();
            const dataDirectory = dataDirectoryForBu(projectRoot, credential, bu);
            ...
            for (const deKey of deKeys) {
                const candidates = await findImportCandidates(dataDirectory, deKey);
                const { paths: importPaths } = await resolveImportSet(candidates);
                if (importPaths.length === 0) {
                    log.error(
                        `No import file (csv/tsv/json) found for DE "${deKey}" under "${path.resolve(dataDirectory)}"`,
                    );
                    return 1;
                }
                ...
                const sourceLabel = importPaths.map((p) => `"${path.resolve(p)}"`).join(', ');
                log.info(`Imported: ${sourceLabel} (${n} rows) -> DE ${deKey}`);
        ...
        for (const customerKey of keysFromFiles) {
            ...
            const sourceLabel = importPaths.map((p) => `"${path.resolve(p)}"`).join(', ');
            log.info(`Imported: ${sourceLabel} (${n} rows)`);
