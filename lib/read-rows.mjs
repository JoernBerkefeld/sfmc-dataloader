...
export async function readRowsFromImportPaths(filePaths, format) {
    ...
    for (let index = 1; index < filePaths.length; index++) {
        rest.push(
            ...(await readRowsFromFile(filePaths[index], format, {
...
export async function* streamRowsFromImportPaths(filePaths, format) {
    ...
    for (let index = 0; index < filePaths.length; index++) {
        if (index === 0) {
            let isSawRow = false;
            const firstFileRows = streamRowsFromFile(filePaths[0], format);
            for await (const row of firstFileRows) {
                if (!isSawRow) {
            ...
            yield* streamRowsFromFile(filePaths[index], format, {
